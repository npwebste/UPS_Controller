...
PV_State = 0 # State of the solar PV, 0 represents open circuit, 1 represents closed circuit
N = 2 # Number of iterations for voltage check
...
def PWM_Controller(arg):
    ...
    if (Solar_Actual_Volts < Parameters.Solar_VDC_Max) and (Solar_Actual_Volts > Parameters.Solar_VDC_Min):
        ...
        D_PID_OLD = D_PID # Update the duty cycle variable for the next iteration
        time.sleep(.25)
    else:
        logger.warning('Solar voltage out of acceptable range, cannot close in solar PV DC relay')
    time.sleep(.05)

    # Measure updated DC link voltage after switching event
    DC_Volts = PWM_Measure_Voltage('DC_Link')
    DC_Link_Actual_Volts = DC_Volts/Parameters.Voltage_Multiplier

    # Check if voltage is operating within acceptable range
    for n in range(N):
