...
for t in time:
	...
	model.addCons(P_In_S[t]==((Theta_VFD[t]/Theta_Max)*((1-Alpha[t])*P_Solar_Ref[t])), name="Solar Power Equality")
	
	model.addCons(P_Sys[t] == (((rho*Ap)/(Am*Km))*(Vf[t]-Vr[t]))+P_Osmotic, name="System Pressure Equality")
	...
	model.addCons(Vf[t] == (Theta_VFD[t]*Vp)/Ap, name="Flow Rate Calc")
        ...
        model.addCons(P_RO[t] == (1/eta)*((P_Sys[t]*Fp[t]/Yopt[t])+.5*((Fp[t]*Fp[t]*Fp[t])/((Yopt[t]*Yopt[t]*Yopt[t])*(Ap*Ap)))*rho), name="RO Power Calc")
