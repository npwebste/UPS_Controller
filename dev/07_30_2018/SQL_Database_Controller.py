...
def SQL_Database_Controller(arg):
    ...
    # Assign VFD power to the source being used
    ...
    try:
        ...
        c = conn.cursor()
        c.execute("INSERT INTO UPS_DB(Date,Solar_Voltage, DC_Link_Voltage, Duty_Cycle, VFD_Freq, VFD_Volt, VFD_Amps, VFD_Power, Grid_Power, Solar_Power, VFD_BusVolt, VFD_Temp) VALUES(datetime('now'),?,?,?,?,?,?,?,?,?,?,?)",(Solar_Voltage,DC_Link_Voltage, Duty_Cycle,VFD_Freq,VFD_Volt,VFD_Amps,VFD_Power, Grid_Power, Solar_Power, VFD_BusVolt,VFD_Temp))
        conn.commit()
