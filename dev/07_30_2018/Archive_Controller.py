...
def Archive_Controller(arg):
    ...
    # Call function to write SQL database to CSV file
    CSV_Write()
    ...
    try:
        os.chmod('UPS_Messages.log', 0o777)
        os.chmod('UPS_DB.csv', 0o777)
    ...
    try:
        ...
        c = conn.cursor()
        c.execute("DELETE FROM UPS_DB WHERE Date <= datetime('now','-1 day')")# Delete sql database older than one week
        conn.commit()
        conn.close()
    except:
